  ...
  <div class="ai-image-swap-container">
    ...
    <div class="swap-content">
      ...
      <div class="left-panel">
        ...
        <div class="step-section" :class="{ 'active-step': true }">
          <div class="step-header">
            ...
            <span class="step-desc">上传要生成线稿图的鞋款</span>
      ...
      <div class="right-panel">
        ...
        >
          {{ shoeStore.aiTaskStatus === 'running' ? '生成中...' : '生成线稿图' }}
        </el-button>
      </div>
...
import { uploadImage, feedbackImage, xgt, } from '../../api/file'
import type { UploadImageResponse, XgtRequest } from '../../api/file'
...
// 新增：跟踪当前处理阶段
const currentProcessingStage = ref<'idle' | 'matting' | 'lineart'>('idle')

// 新增：跟踪是否正在处理线稿图任务
const isProcessingLineArtTask = ref(false)
  ...
  const doUpload = (file: File) => {
    uploadFile(file, (_, imageId) => {
      if (imageId) {
        ...
        originalImageId.value = String(imageId);
      }
...
const handleImageEdited = (editedImageUrl: string, imageId?: number) => {
  ...
  if (imageId) {
    mainImageName.value = String(imageId);
    originalImageId.value = String(imageId); // 关键：同步ID为最新编辑后的图片ID
    
    // 更新全局store状态，确保使用最新的图片ID
    shoeStore.setOriginalImageId(imageId);
    console.log('🌐 LineArt - 编辑后更新全局图片ID:', imageId);
  }
  showEditDialogMain.value = false;
};
...
watch(() => shoeStore.aiTaskImages, (newImages) => {
  if (newImages && newImages.length > 0) {
    // 只有在线稿图任务进行中时才显示结果
    if (isProcessingLineArtTask.value) {
      ...
      ElMessage.success('线稿图生成成功')
      isProcessingLineArtTask.value = false // 重置任务状态
    } else {
      // 其他抠图任务的结果，不显示在线稿图组件中
      console.log('收到其他抠图任务结果，不显示在线稿图组件中')
    }
  }
}, { deep: true })
...
const handleGenerate = async () => {
  ...
  isViewingResults.value = false;
  resultImages.value = [];
  isProcessingLineArtTask.value = true; // 设置为线稿图任务进行中
  ...
  try {
    ...
    const requestData: XgtRequest = {
      imageId: imageIdToUse
    }
    
    const response = await xgt(requestData)
    ...
    if (response.code === 0 || response.code === 200) {
      ...
      if (result && result.promptId && result.clientId && result.server) {
        ...
        // 启动WebSocket连接
        startAiTaskWs(result.clientId, result.server, result.promptId, 'line-art')
      } else {
        // 直接处理结果（兼容旧版本）
        if (result && result.viewUrls && Array.isArray(result.viewUrls)) {
          ...
          if (resultImages.value.length > 0) {
            ...
            ElMessage.warning('线稿图生成成功但未获得图片')
          }
        } else {
      ...
      throw new Error(response.msg || '线稿图生成失败')
    }
  } catch (error: any) {
    console.error('线稿图失败:', error)
    ElMessage.error('线稿图失败: ' + (error.message || '未知错误'))
    isProcessingLineArtTask.value = false // 重置任务状态
  } finally {
