  ...
  <div class="ai-image-swap-container">
    ...
    <div class="swap-content">
      ...
      <div class="left-panel">
        ...
        <div class="step-section" :class="{ 'active-step': true }">
          <div class="step-header">
            ...
            <span class="step-desc">上传要去除水印的鞋款</span>
      ...
      <div class="right-panel">
        ...
        >
          {{ shoeStore.aiTaskStatus === 'running' ? '去除中...' : '去除水印' }}
        </el-button>
      </div>
...
import { uploadImage, feedbackImage, qsy, } from '../../api/file'
import type { UploadImageResponse, QsyRequest } from '../../api/file'
...
// 新增：跟踪是否正在处理去除水印任务
const isProcessingWatermarkTask = ref(false)
  ...
  const doUpload = (file: File) => {
    uploadFile(file, (_, imageId) => {
      if (imageId) {
        ...
        originalImageId.value = String(imageId); // 保存原始上传的图片ID
...
const handleImageEdited = (editedImageUrl: string, imageId?: number) => {
  console.log('🖼️ 图片编辑完成:', {
    editedImageUrl: editedImageUrl.substring(0, 50) + '...',
    imageId,
    originalImageId: originalImageId.value,
    oldMainImageName: mainImageName.value
  });
  
  mainImage.value = editedImageUrl;
  if (imageId) {
    mainImageName.value = String(imageId); // 更新为编辑后的图片ID
    ...
    console.log('✅ 已更新图片ID:', {
      newMainImageName: mainImageName.value,
      originalImageId: originalImageId.value,
      全局当前图片ID: shoeStore.currentImageId
    });
  } else {
    console.log('⚠️ 没有收到编辑后的图片ID');
  }
  showEditDialogMain.value = false;
};
...
const handleClearImage = () => {
  ...
  isProcessingWatermarkTask.value = false // 重置任务状态
}
...
watch(() => shoeStore.aiTaskImages, (newImages) => {
  if (newImages && newImages.length > 0) {
    // 只有在去除水印任务进行中时才显示结果
    if (isProcessingWatermarkTask.value) {
...
const handleGenerate = async () => {
  ...
  try {
    ...
    const requestData: QsyRequest = {
      imageId: imageIdToUse
    }
    
    const response = await qsy(requestData)
    ...
    if (response.code === 0 || response.code === 200) {
      ...
      if (result && result.promptId && result.clientId && result.server) {
        ...
        // 启动WebSocket连接
        startAiTaskWs(result.clientId, result.server, result.promptId, 'watermark-remove')
      } else {
        // 直接处理结果（兼容旧版本）
        if (result && result.viewUrls && Array.isArray(result.viewUrls)) {
          ...
          if (resultImages.value.length > 0) {
            ...
            ElMessage.warning('去除水印成功但未获得图片')
          }
        } else {
      ...
      throw new Error(response.msg || '去除水印失败')
    }
  } catch (error: any) {
    console.error('去除水印失败:', error)
    ElMessage.error('去除水印失败: ' + (error.message || '未知错误'))
    isProcessingWatermarkTask.value = false // 重置任务状态
  } finally {
