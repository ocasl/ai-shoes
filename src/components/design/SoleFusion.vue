...
// 添加错误状态监听
watch(() => shoeStore.aiTaskStatus, (newStatus) => {
  if (newStatus === 'error' && isProcessingPartialModifyTask.value) {
...
const handleGenerate = async () => {
  ...
  try {
    ...
    const response = await xdhh(requestData);
    console.log('收到局部修改响应:', response);

    // 检查API响应格式 - 直接返回taskId
