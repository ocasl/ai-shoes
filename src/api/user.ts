 ...
 * 更新用户信息
 */
export async function updateUserInfo(userData: UpdateUserRequest): Promise<ApiResponse<User>> {
  try {
 ...
 * 修改密码
 */
export async function changePassword(passwordData: ChangePasswordRequest): Promise<ApiResponse<void>> {
  try {
 ...
 */
export async function upgradeUser(upgradeData: { inviteCode: string }): Promise<ApiResponse<User>> {
  try {
 ...
 */
export async function terminateSession(sessionId: string): Promise<ApiResponse<void>> {
  try {
...
export async function checkUsernameAvailability(username: string): Promise<ApiResponse<{ available: boolean }>> {
  try {
    const response = await request.get(`/check-username?username=${encodeURIComponent(username)}`)
 ...
 * 短信验证码登录
 */
export async function smsLogin(credentials: { phone: string; code: string }): Promise<ApiResponse<LoginResponse>> {
  try {
